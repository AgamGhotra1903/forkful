...
export const addMenuItem = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (!uploadResult?.url) {
    return res.status(500).json({ message: "Image upload failed — no URL returned" });
  }
  ...
  // Publish to menuitem.embed queue
  try {
    const channel = getChannel();
    channel.sendToQueue(
      "menuitem.embed",
      Buffer.from(JSON.stringify({ itemId: item._id })),
      { persistent: true }
    );
  } catch (queueError) {
    console.error("[MenuItems Controller] Failed to publish menuitem.embed event:", queueError);
  }
